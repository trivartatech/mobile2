...
| `0033-driver-mobile-api-fix.patch` | Wires up the driver mobile app's missing API endpoints. Without this patch, every GPS push 404s, the vehicle picker is empty, and drivers can't mark student boarding. Adds `GET /mobile/transport/driver/vehicles`, `POST /mobile/transport/driver-tracking/update`, `POST /mobile/transport/driver-tracking/stop`, plus two new transport-attendance endpoints (`GET .../attendance/students` and `POST .../attendance/mark`) that fire SMS / WhatsApp / push to parents on save. Driver/conductor scope is enforced server-side. Pure controller / route change — no migration. Mobile UI ships with next Expo build. |
| `0034-fix-driver-dropdown-empty.patch` | Hotfix: the **Edit Vehicle** modal's "Driver" dropdown was empty even when driver users existed. The query required a `staff_designation` row with name='driver', but most installations identify drivers via `users.user_type='driver'` and never set up the designation. Now matches staff whose linked user has `user_type='driver'` OR (legacy fallback) whose designation name equals 'driver'. Pure controller change — no migration, no Vue rebuild. |
| `0035-fix-driver-dashboard-no-vehicles.patch` | Hotfix follow-up to 0033: the driver mobile dashboard kept showing "no vehicles assigned" even after the admin assigned a vehicle. Root cause was that the dashboard pulled `/mobile/transport/live`, which only returns vehicles with a recent `live_location` row — so drivers who hadn't started GPS tracking yet always saw an empty list. Fix enriches `assignedVehicles()` to include route + stops + student counts + live status; the mobile dashboard switches to this endpoint (next Expo build). Pure controller change — no migration. |
| `0036-gate-keeper-mobile-api.patch` | Wires up the front gate keeper mobile app, which previously had a UI shell but no functional backend (every call 404'd). Adds 7 endpoints under `/mobile/front-office/`: `gate/stats` (today's counters), `gate-passes/verify/{token}` (QR resolution + next valid action), `gate-passes/{id}/exit`, `gate-passes/{id}/entry` (state-machine transitions stamping exit_time / return_time), `visitors` (today list), `visitors` POST (walk-in log with in_time = now), `visitors/{id}/exit` (out_time stamp). School scope enforced; valid status transitions return 422 on bad input. Pure controller + routes — no migration. Mobile UI (QR scanner, log-visitor screen, refresh, notifications tab) ships with next Expo build. |
